...
def log_offline_data(gps_data):
    ...
    try:
        ...
        json_file = os.path.join(GPS_DATA_DIR, "offline_gps_data.json")
        with open(json_file, 'a') as f:
            json.dump(gps_data, f)
            f.write('\n')
        logger.info(f"Logged offline GPS data to {json_file}")
    except Exception as e:
...
async def send_offline_data(websocket):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Cleared offline log file {json_file}")
        except Exception as e:
...
async def main():
    ...
    try:
        ...
        end_message = f"\n=== Application Ended at: {end_time} (Started at: {app_start_time}) ===\n"
        logger.info(end_message.strip())

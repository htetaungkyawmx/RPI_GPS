    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith("GPS Data (Real-Time):"):
                ...
                try:
                    data["gps_data"][current_index]["altitude"] = float(alt_str) if lat_str != "Unknown" else None
                except ValueError:
                ...
                if connected_clients:
                    for client in connected_clients.copy():
                        try:
                            ...
                            connected_clients.discard(client)
                if not external_ws_connected:
                    log_offline_data(parsed_data)
                logger.info(f"Broadcasted parsed GPS data: {parsed_data}")
            gps_data_queue.task_done()
        except Empty:
            await asyncio.sleep(0.1)
        except Exception as e:
